...
struct Configs {
    ...
    int change_color, swap_sq,draw_square,help,info;
...
void kurs(struct Png *image, int x1, int y1) {
        for (int x = 0; x < x1; x++) {
...
int main(int argc, char **argv) {
  ...
  /*  struct Configs config = {0, 0, 0,
    ...
    char *opts = "c1:2:dSl:r:t:f:a:ho:iC";
    struct option longOpts[] = {
            {"color_change",   no_argument,       NULL, 'c'},
            {"color",         required_argument, NULL, '1'},
            {"draw_square",    no_argument,       NULL, 'd'},
            {"swap_sq",        no_argument,       NULL, 'S'},
            {"left_srcCoord",  required_argument, NULL, 'l'},
            {"right_srcCoord", required_argument, NULL, 'r'},
            {"thickness",      required_argument, NULL, 't'},
            {"filling",        required_argument, NULL, 'f'},
            {"square_side",    required_argument, NULL, 'a'},
    ...
    if(config.draw_square == 1){
        draw_square(&image, config.x1, config.y1, config.a,
                    config.l_w, config.c_r, config.c_g, config.c_b,config.filling);
    }
    if(config.swap_sq == 1){
        swap_sq(&image, &copy, config.x1, config.y1,
                config.x2, config.y2, config.opt);
    }
    if(config.change_color == 1){
        color_change(&image, config.c_r, config.c_g, config.c_b);
    }
    */
